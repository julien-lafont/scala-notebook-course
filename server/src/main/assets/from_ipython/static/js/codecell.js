...
var IPython = (function (IPython) {
    ...
    CodeCell.prototype.create_element = function () {
        var cell =  $('<div></div>').addClass('cell code_cell hbox');
        cell.attr('tabindex','2');

        var content = $('<div></div>').addClass('content box-flex1');

        var optionBar = this.create_option_bar();
        cell.append(optionBar);
        ...
        this.code_mirror = CodeMirror(input_area.get(0), {
            ...
            onChange: $.proxy(function() {this.notebook.set_dirty();},this) // Mark notebook dirty for auto-save.
        });
        input.append(input_area);
        var output = $('<div></div>').addClass('output');
        content.append(input).append(output);
        cell.append(content);
        this.element = cell;
        this.collapse();
    };

    CodeCell.prototype.create_option_bar = function () {
    	var that = this;
    ...
    CodeCell.prototype.remove_and_cancel_tooltip = function() {
        ...
        this.tooltip_msg_id = undefined;
    }

    CodeCell.prototype.finish_tooltip = function (reply, msg_id) {
        if (msg_id !== this.tooltip_msg_id) return;
        ...
        //TODO: explicitly reference 'tooltip', do not rely on global state
    ...
    CodeCell.prototype.finish_completing = function (matched_text, matches) {
        ...
        var key = { tab:9,
                    ...
                    dismissAndAppend : function (code)
                        {
                        // CY: This is almost certainly broken -- the keypress code (passed in as code) is different
                        // from the code returned by charCodeAt (which returns unicode codes)
                        chararr = '()[]+-/\\. ,=*'.split("");
        ...
        if(this.notebook.smart_completer)
        {
            ...
            for(var i = 0 ; i<matches.length ; ++i){
                if(matches[i].completion.substr(-1) === '='){
        ...
        var complete_with = function(matches,typed_text,autopick,event)
        {
            ...
            if (matches.length < 1) {
                ...
                insert(matches[0].completion,event);
                if(event != null){
                event.stopPropagation();
                event.preventDefault();
                }
            ...
            for (var i = 0; i<matches.length; ++i) {
                var match = matches[i];
                var $option = $('<option/>').html(match.completion).attr('value', match.completion).data('match_data', match);
        ...
        ff = sharedStart($.map(matches, function(m) { return m.completion; }));
        fastForward = ff[0];
        ...
        var downandpress = function (event,press_or_down) {
            ...
            if (code === key.enter || code === key.tab) {
                ...
                filterd = matches.filter(function(x){return re.test(x.completion)});
    ...
    CodeCell.prototype.append_output = function (json, dynamic) {
        ...
        } else if (json.output_type === "pyincomplete") {
            this.handle_pyincomplete(json);
    ...
    CodeCell.prototype.append_pyout = function (json, dynamic) {
        ...
        toinsert.find('div.prompt').addClass('output_prompt').html('[' + n + ']');
    ...
    CodeCell.prototype.handle_pyincomplete = function(json) {
        var cellContents = this.element[0];
        var oldBackgroundColor = cellContents.style.backgroundColor;
        cellContents.style.backgroundColor = "red";
        setTimeout(function() {
    ...
    CodeCell.prototype.append_html = function (html, element) {
        ...
        curl(['observable'], function (Observable) {
    ...
    CodeCell.prototype.expand = function () {
        if (this.collapsed) {
            // CY: Ultra-hack to get around printing issue
            this.element.find('div.output').show().removeAttr('style');
            this.collapsed = false;
        };
    };
    ...
    CodeCell.prototype.toggle_output = function () {
    	this.show_sections(undefined, this.collapsed);
    };

    CodeCell.prototype.hide_input = function () {
        this.element.addClass('output_only');
        this.showInput = false;
    };

    CodeCell.prototype.show_input = function () {
    ...
    CodeCell.prototype.set_input_prompt = function (number) {
        ...
        this.element.find('div.input_prompt').html('[' + ns + ']');
    };
